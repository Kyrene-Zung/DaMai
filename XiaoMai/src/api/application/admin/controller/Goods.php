...
//        数据库的优化方法，第一个：需要哪些字段就只查询哪些字段
       ...
       if (input("cate_id")) {
          $search_a["m.cate_id"]=array("=",input("cate_id"));
       }
        $goods_list=$m
            ->alias('m')
            ->field('m.goods_id,m.goods_title,m.goods_price,s.cate_name')
            ->join("dm_cate s","m.cate_id=s.cate_id")
            ->where($search_a)
            ->order(" m.goods_id desc ")
            ->paginate(10);
    ...
    public function recommed()
    {
      ...
      if (!empty($id_a)) {
          foreach ($id_a as $key => $value) {
              db('goods')->where("goods_id=$value")->update(['type'=>input('type')]);
          }

...
if(params.length==0){
	...
	superagent.post(url).end(function(error,rtnData){
		...
		//$是整个html 找盒子 遍历
		//获取子分类
		$('#subcategory_filter_id .search_city_num .clear li').each(function(index,ele){
			var subClass=$(ele).find('a').text();
			subClassArr.push(subClass);
		})
		//获取该分类的商品
		// $('.search_subClass_all li').each(function(index,ele){
		...
		fs.writeFile('subClass.json',JSON.stringify(subClassArr));
